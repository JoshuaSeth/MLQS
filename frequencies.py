...
class FourierTransformation:
    ...
    def abstract_frequency(self, data_table, columns, window_size, sampling_rate):
        ...
        for col in tqdm(columns):
            ...
            # tqdm.write('foudn freqeuncies', frequencies[:10])
            frame = pd.DataFrame(frequencies)
            temp_df[collist] = frame
            temp_df.index = data_table.index

            data_table=data_table.merge(temp_df, how='inner', left_on='time', right_on='time')
